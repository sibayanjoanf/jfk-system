...
const SystemSetting: React.FC = () => {
  const [activeTab, setActiveTab] = useState<
    "categories" | "notifications" | "company" | "storefront" | "database"
  >("categories");
  const [activeStorefrontTab, setActiveStorefrontTab] = useState<
    "announcements" | "showcase" | "faq"
  >("announcements");
  ...
  const [categories, setCategories] = useState<Category[]>([
    {
      ...
      expanded: true,
      subCategories: [
        { id: "1a", name: "Glossy Porcelain", thumbnail: "" },
        { id: "1b", name: "Matt Glazed Porcelain", thumbnail: "" },
        { id: "1c", name: "Non Skid Tile", thumbnail: "" },
        { id: "1d", name: "Glazed Porcelain", thumbnail: "" },
        { id: "1e", name: "Rustic Porcelain", thumbnail: "" },
        { id: "1f", name: "Glossy Ceramic Tile", thumbnail: "" },
      ],
    },
    {
      id: "2",
    ...
    },
    {
      ...
      expanded: false,
      subCategories: [
        { id: "3a", name: "Bathroom Fixtures", thumbnail: "" },
        { id: "3b", name: "Electrical Fixtures", thumbnail: "" },
        { id: "3c", name: "Hardware Fixtures", thumbnail: "" },
        { id: "3d", name: "Kitchen Fixtures", thumbnail: "" },
      ],
    },
  ]);

  const [showAddCategory, setShowAddCategory] = useState(false);
  const [newCategory, setNewCategory] = useState({ name: "", thumbnail: "" });
  const [showAddSubMap, setShowAddSubMap] = useState<Record<string, boolean>>(
    {},
  );
  const [newSubMap, setNewSubMap] = useState<
    Record<string, { name: string; thumbnail: string }>
  >({});
  ...
  const addSubCategory = (catId: string) => {
    ...
    setNewSubMap((prev) => ({ ...prev, [catId]: { name: "", thumbnail: "" } }));
  ...
  const handleCategoryThumbnail = (catId: string, file: File) => {
    const url = URL.createObjectURL(file);
    setCategories((prev) =>
      prev.map((c) => (c.id === catId ? { ...c, thumbnail: url } : c)),
    );
  };

  const handleSubThumbnail = (catId: string, subId: string, file: File) => {
    const url = URL.createObjectURL(file);
    setCategories((prev) =>
      prev.map((c) =>
        c.id === catId
  ...
  const handleNewCategoryThumbnail = (file: File) => {
    const url = URL.createObjectURL(file);
    setNewCategory((prev) => ({ ...prev, thumbnail: url }));
  };

  const handleNewSubThumbnail = (catId: string, file: File) => {
    const url = URL.createObjectURL(file);
    setNewSubMap((prev) => ({
      ...prev,
      [catId]: { ...prev[catId], thumbnail: url },
    }));
  };
  ...
  const saveSubCategoryName = (catId: string, subId: string) => {
  ...
  const [announcements, setAnnouncements] = useState<Announcement[]>([
    {
      id: "1",
      text: "Special Offer: Get 10% off on all Floor Tiles this month!",
      active: false,
    },
    { id: "2", text: "Free delivery for orders over ₱50,000", active: true },
    {
      id: "3",
      text: "Visit our showrooms in Barit, Bulangon, and Rizal",
      active: true,
    },
    {
      id: "4",
      text: "Quality Tile and Stone Builders since 2009",
      active: true,
    },
  ]);
  ...
  const [showcaseImages, setShowcaseImages] = useState<ShowcaseImage[]>([
    ...
    { id: "3", image: "", product: "BMD65008", category: "Glossy Porcelain" },
    {
      id: "4",
      image: "",
      product: "One Piece Watercloset",
      category: "Bathroom Fixtures",
    },
    {
      id: "5",
      image: "",
      product: "KARAGIN 6658",
  ...
  const addFaq = () => {
    ...
    setFaqs((prev) => [
      ...prev,
      { id: Date.now().toString(), ...newFaq, open: false },
    ]);
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between w-full px-4 py-3.5 text-sm font-medium transition-all border-b border-gray-50 last:border-0 ${isActive ? "bg-red-50 text-red-600" : "text-gray-600 hover:bg-gray-50"}`}
              >
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Manage how products are organized. Each category can have
                    multiple sub-categories with their own thumbnails.
                  </p>
                </div>
                <button
              ...
              {showAddCategory && (
                <div className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-100 space-y-3">
                  ...
                  <div className="flex items-center gap-4">
                    {/* Thumbnail upload */}
                    <div
                      onClick={() => newCatThumbRef.current?.click()}
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        e.target.files?.[0] &&
                        handleNewCategoryThumbnail(e.target.files[0])
                      }
                    />
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400 mt-1.5">
                        Click the box to upload a thumbnail image.
                      </p>
                    </div>
                  </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            e.target.files?.[0] &&
                            handleCategoryThumbnail(cat.id, e.target.files[0])
                          }
                        />
                        ...
                        <div className="flex-1 min-w-0">
                          {editingCategory === cat.id ? (
                            <input
                              ...
                              onKeyDown={(e) =>
                                e.key === "Enter" && saveCategoryName(cat.id)
                              }
                        ...
                        <div className="px-4 pb-3 pt-2 space-y-2 bg-white">
                          {cat.subCategories.map((sub) => {
                            ...
                            return (
                              <div
                                key={sub.id}
                                className="flex items-center gap-3 pl-6 pr-2 py-2.5 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors"
                              >
                                <ChevronRight
                                  size={12}
                                ...
                                <div className="flex-1 min-w-0">
                                  {editingSubCategory === sub.id ? (
                                    <input
                                      ...
                                      onBlur={() =>
                                        saveSubCategoryName(cat.id, sub.id)
                                      }
                                      onKeyDown={(e) =>
                                        e.key === "Enter" &&
                                        saveSubCategoryName(cat.id, sub.id)
                          ...
                          {showAddSubMap[cat.id] ? (
                            <div className="pl-6 pr-2 py-3 rounded-lg border border-dashed border-red-200 bg-red-50/30 space-y-2">
                              <div className="flex items-center gap-3">
                                ...
                                >
                                  {newSubMap[cat.id]?.thumbnail ? (
                                    <img
                                      src={newSubMap[cat.id].thumbnail}
                                      alt=""
                                      className="w-full h-full object-cover"
                                    />
                                  ) : (
          ...
          {activeTab === "notifications" && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <p className="text-xs text-gray-400 mb-8 leading-relaxed">
                Important updates about orders or account may still be sent to
                you, even with notifications disabled.
              </p>
                ...
                {storefrontTabs.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 flex-1 justify-center px-4 py-2 text-xs font-medium rounded-lg transition-colors ${activeStorefrontTab === tab.key ? "bg-red-600 text-white shadow-sm" : "text-gray-500 hover:bg-gray-50"}`}
                    >
                      <Icon size={13} />
                      {tab.label}
                    </button>
              ...
              {activeStorefrontTab === "announcements" && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  <div className="mb-6">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Banners shown to customers on your storefront.
                    </p>
                  </div>
                  <div className="flex gap-2 mb-6">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors"
                    >
                      <Plus size={14} />
                      Add
                    </button>
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            {faq.category && (
                              <span className="px-2 py-0.5 text-xs bg-red-50 text-red-600 rounded-full font-medium shrink-0">
                                {faq.category}
                              </span>
                            )}
                            <p className="text-sm font-medium text-gray-900 truncate">
...
const Toggle = ({
  ...
  >
    <div
      className={`w-9 h-5 flex items-center rounded-full px-0.5 transition-colors duration-200 ${enabled ? "bg-red-600" : "bg-gray-200"}`}
    >
      <div
        className={`bg-white w-4 h-4 rounded-full shadow-sm transform transition-transform duration-200 ${enabled ? "translate-x-4" : "translate-x-0"}`}
      />
    </div>
    {label && (
