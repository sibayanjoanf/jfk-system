...
export default function InquiryFormPage() {
  ...
  const inputStyles = "border-gray-200 bg-transparent h-12 text-sm focus-visible:ring-1 focus-visible:ring-red-500 focus-visible:border-red-500";
  ...
  const handlePhoneFocus = () => { if (!phone) setPhone('+63'); };
  const handlePhoneBlur = () => { if (phone === '+63') setPhone(''); };
          ...
          <div className="container mx-auto px-4">
            <div className="flex h-16 items-center justify-between">
              <Link href="/" className="flex items-center space-x-2">
                <img src={logo} alt="JFK Logo" className="h-9 w-auto object-contain" />
      ...
      <main className="pt-10">
        <div className="container mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-start pb-10">
          ...
          <div className="flex flex-col gap-5 order-2 lg:order-1 lg:sticky lg:top-32 self-start">
            <div>
              <h1 className="font-semibold text-xl md:text-2xl text-gray-900">Inquiry Form</h1>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <Select onValueChange={(val) => { setDeliveryPref(val); setErrors(p => ({ ...p, deliveryPref: '' })); }}>
                  <SelectTrigger className={cn("w-full py-6 bg-transparent text-sm border-gray-200 text-gray-900", errors.deliveryPref && 'border-red-400')}>
              ...
              <div>
                <Select onValueChange={(val) => { setPaymentPref(val); setErrors(p => ({ ...p, paymentPref: '' })); }}>
                  <SelectTrigger className={cn("w-full py-6 bg-transparent text-sm border-gray-200", errors.paymentPref && 'border-red-400')}>
            ...
            <div className="flex flex-wrap gap-4 pt-4 border-t border-gray-100 text-xs text-gray-400 justify-center lg:justify-start">
              <Link href="/terms-and-conditions" className="hover:text-red-600 transition-colors">Terms & Conditions</Link>
          ...
          <div className="flex flex-col order-1 lg:order-2 lg:sticky lg:top-32 self-start">
            ...
            >
              <div className="flex items-center gap-2">
                <h2 className="font-semibold text-xl text-gray-900">Your Order</h2>
                <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full text-gray-500">
                  {items.length} {items.length === 1 ? 'item' : 'items'}
                </span>
              </div>
              {isOrderOpen ? <ChevronUp size={15} className="text-gray-400" /> : <ChevronDown size={15} className="text-gray-400" />}
            </button>

            {/* Desktop heading */}
            <div className="hidden lg:block mb-4">
              <p className="text-xs uppercase tracking-widest text-gray-400 mb-1">Your Order</p>
              <h2 className="font-semibold text-xl md:text-2xl text-gray-900">
                {items.length} {items.length === 1 ? 'item' : 'items'}
              </h2>
            </div>

            <div className={cn("flex-col lg:flex", isOrderOpen ? "flex" : "hidden")}>
              ...
              <p className="text-xs text-gray-400">Final pricing will be confirmed upon processing your inquiry.</p>
            </div>
          </div>
        </div>
      </main>
