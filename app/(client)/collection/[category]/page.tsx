  ...
  const displayProducts = useMemo(() => {
    ...
    if (activeSubCategory) {
      result = result.filter(
        (p) => p.sub_categories?.name === activeSubCategory,
      );
    }

    if (inStockOnly) {
      result = result.filter((p) =>
        p.product_variants?.some((v) => v.stock_qty > 0),
      );
    }
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="relative h-[20vh] md:h-[30vh] overflow-hidden">
        ...
        <div className="container relative z-10 mx-auto px-4 h-full flex items-center justify-center">
          <h1
            className={cn(
              "text-5xl md:text-7xl font-bold text-white text-center",
              "windsong-medium",
            )}
          >
            {categoryName}
                          ...
                          <li key={sub.id}>
                            {isCatActive ? (
                              <button
                                ...
                                className={cn(
                                  "block w-full text-left py-1 text-sm transition-colors hover:text-red-600",
                                  activeSubCategory === sub.name
                              ...
                              <Link
                                ...
                                className={cn(
                                  "block py-1 text-sm transition-colors hover:text-red-600",
