...
interface HomePageProps {
  tilesProducts: Product[];
  stonesProducts: Product[];
  fixturesProducts: Product[];
}
      ...
      >
        <CarouselContent>
          {products.map((product) => (
            // basis-1/2 = 2 cards on mobile
            // md:basis-1/3 = 3 cards on tablet
            // lg:basis-1/5 = 5 cards on desktop
...
export default function HomePage(
    { tilesProducts, stonesProducts, fixturesProducts }: HomePageProps
){

  const [Categories, setCategories] = useState<Categories[]>([]);
  const [Showcase, setShowcase] = useState<ShowcaseProducts[]>([]);
  ...
  useEffect(() => {
    ...
    fetch('/api/showcase')
      ...
      .catch((err) => console.error("Failed to fetch Showcase Products:", err));
  }, []);
          ...
          >
            <CarouselContent>
                {Showcase.map((showcase, index) => {
                  const productURL = `/collection/${showcase.products?.sub_categories?.categories?.name?.toLowerCase().replace(/\s+/g, '-')}/${showcase.products?.sub_categories?.name?.toLowerCase().replace(/\s+/g, '-')}/${showcase.product_name.toLowerCase().replace(/\s+/g, '-')}`;
                      ...
                      <Link href={productURL}>
                        <div className="group relative aspect-[3/4] overflow-hidden rounded-lg bg-gray-100 cursor-pointer">
                          ...
                          <div className="absolute inset-x-0 bottom-0 p-4 translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                            <p className="text-white text-sm font-medium">{showcase.product_name}</p>
                            <p className="text-gray-300 text-xs">{showcase.products?.sub_categories?.name}</p>
                          </div>
                        </div> 
                      </Link>
                    </CarouselItem>
